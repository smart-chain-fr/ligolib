...
let () = Log.describe("[Vesting - RevokeAddress - FA2] test suite")

let metadata_empty : (string, bytes) big_map = (Big_map.empty : (string, bytes) big_map)



let test_failure_revoke_address_by_admin_before_start =
    ...
    let result = Vesting_helper.revoke_beneficiary(bob, 0tez, vesting.contr) in
    Assert.string_failure result Vesting.Errors.vesting_not_started

let test_failure_revoke_address_not_revocable =
    let accounts = Bootstrap.boot_accounts(Vesting_helper.zero_timestamp) in
    let (admin, alice, bob) = accounts in
    let token_id = 0n in
    let fa2 = Bootstrap.boot_fa2(token_id, admin) in
...
let test_failure_revoke_address_by_unauthorized =
    let accounts = Bootstrap.boot_accounts(Vesting_helper.zero_timestamp) in
    let (admin, alice, bob) = accounts in
    let token_id = 0n in
    let fa2 = Bootstrap.boot_fa2(token_id, admin) in
...
let test_failure_revoke_address_vesting_revoked =
    let accounts = Bootstrap.boot_accounts(Vesting_helper.zero_timestamp) in
    let (admin, alice, bob) = accounts in
    let token_id = 0n in
    let fa2 = Bootstrap.boot_fa2(token_id, admin) in
...
let test_failure_revoke_address_twice =
    let accounts = Bootstrap.boot_accounts(Vesting_helper.zero_timestamp) in
    let (admin, alice, bob) = accounts in
    let token_id = 0n in
    let fa2 = Bootstrap.boot_fa2(token_id, admin) in
    ...
    let () = Vesting_helper.start_success(unit, 0tez, vesting.contr) in
    let () = Vesting_helper.assert_vesting_revoked(vesting.taddr, false) in
    let () = Vesting_helper.revoke_beneficiary_success(bob, 0tez, vesting.contr) in
    let () = Vesting_helper.assert_beneficiary_revoked(vesting.taddr, bob, true) in
    let result = Vesting_helper.revoke_beneficiary(bob, 0tez, vesting.contr) in
    Assert.string_failure result Vesting.Errors.address_already_revoked

let test_success_revoke_address_by_admin_after_start =
    let accounts = Bootstrap.boot_accounts(Vesting_helper.zero_timestamp) in
    let (admin, alice, bob) = accounts in
    let token_id = 0n in
    let fa2 = Bootstrap.boot_fa2(token_id, admin) in
    ...
    let vesting = Bootstrap.boot_vesting(admin, FA2(fa2.addr), token_id, beneficiaries, 10000n, True, (None : timestamp option)) in
    let () = FA2_helper.update_operators_success([Add_operator({owner=admin; operator=vesting.addr; token_id=token_id})], fa2.contr) in
    let () = FA2_helper.assert_user_balance(fa2.taddr, admin, 0n, 1000n) in

    let () = Test.set_source admin in
    let () = Vesting_helper.start_success(unit, 0tez, vesting.contr) in
    let () = FA2_helper.assert_user_balance(fa2.taddr, admin, 0n, 970n) in
    let () = Vesting_helper.assert_vesting_revoked(vesting.taddr, false) in
    let () = Vesting_helper.revoke_beneficiary_success(bob, 0tez, vesting.contr) in
    let () = FA2_helper.assert_user_balance(fa2.taddr, admin, 0n, 980n) in
    Vesting_helper.assert_beneficiary_revoked(vesting.taddr, bob, true)
