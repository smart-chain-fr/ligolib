...
let create_operation (storage : storage_multisig) (params : operation_params) : return = // Amount can be zero
    ...
    let new_operation_counter : nat = storage.operation_counter + 1n in
    let new_operation : operation_send = { target_fa12  = params.target_fa12;
                                           ...
                                           number_of_signer = 1n;
                                           executed = false; } in
    let new_operation_map : (nat, operation_send) big_map = Big_map.add new_operation_counter new_operation storage.operation_map in
...
let sign (storage : storage_multisig) (operation_counter : nat) : return =
    ...
    if Set.size updated_approved_signers >= storage.threshold then
        let updated_operation : operation_send = { target_operation with approved_signers = updated_approved_signers;
                                                                         ...
                                                                         executed         = true } in
        let final_operation : operation = sendFA12 target_operation.token_amount target_operation.target_to target_operation.target_fa12 in
        ...
        let final_storage : storage_multisig = { storage with storage_multisig.operation_map = updated_operation_map } in
        ([final_operation], final_storage)
    else
        let updated_operation : operation_send = { target_operation with approved_signers: updated_approved_signers;
                                                                         number_of_signer: target_operation.number_of_signer + 1n } in
        let final_operation : operation = 
        (no_operation, final_storage)    
