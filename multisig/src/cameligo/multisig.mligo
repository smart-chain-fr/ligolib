...
let sendFA2 (transfer : T.FA2.transfer) (target_fa2_address : address) : operation = 
    ...
    | Some c -> c
    | None -> (failwith E.unknown_reward_token_entrypoint: T.FA2.transfer contract)
    in
...
let create_proposal(params : T.proposal_params)  (storage : T.storage_multisig) : T.return = // Amount can be zero
    ...
    let new_proposal : T.proposal = { target_fa2  = params.target_fa2;
                                           ...
                                           number_of_signer = 1n;
                                           executed = false; } in
    let new_proposal_map : (nat, T.proposal) big_map = Big_map.add new_proposal_counter new_proposal storage.proposal_map in
...
let sign (proposal_number : nat) (storage : T.storage_multisig) : T.return =
    let _check_is_signer : unit  = assert_with_error (Set.mem Tezos.sender storage.signers) E.only_signer in
    ...
    let _check_has_already_signed : unit  = assert_with_error (Set.mem (Tezos.sender : address) target_proposal.approved_signers) E.has_already_signed in
...
type entrypoint_multisig = Create_proposal of T.proposal_params | Sign of nat
