...
indiceAddress = "KT1XJQfDFyJM7gTQqoq57JawLyH7JZ359B5b" # Hardcoded farm address for tests
indiceBAddress = "KT1QgAeibPi5VpBtvV2Mm69hWQSREoPDPRFH" # Hardcoded farm address for tests
compiled_contract_path = "../../compiled/advisor.tz"
initial_lambda = '{ IF_CONS { SWAP ; DROP ; SOME } { NONE int } ; IF_NONE { PUSH string "missing value" ; FAILWITH } { PUSH int 10 ; SWAP ; COMPARE ; LT ; IF { PUSH bool True } { PUSH bool False } }'
initial_storage = ContractInterface.from_file(compiled_contract_path).storage.dummy()
initial_storage["indices"] = [ {"contractAddress":indiceAddress, "viewName": "indice_value"}, {"contractAddress":indiceBAddress, "viewName": "indice_value"} ]
