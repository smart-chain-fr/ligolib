...
let () = Log.describe("[Vesting - Start - FA2] test suite")
...
let test_failure_start_with_zero_duration =
    ...
    let vesting = Bootstrap.boot_vesting(admin, FA2(fa2.addr), token_id, beneficiaries, vesting_duration, True, (None : timestamp option)) in
    let () = FA2_helper.update_operators_success([Add_operator({owner=admin; operator=vesting.addr; token_id=token_id})], fa2.contr) in
