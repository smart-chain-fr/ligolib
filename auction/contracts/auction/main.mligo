...
let setCommissionFee(param, store : Parameter.set_fee_param * Storage.t) : return =
    let _check_admin : unit = Common.assert_only_admin store in
...
let setBidOrder(param, store : Parameter.set_bid_order_param * Storage.t) : return =
    ...
    let _check_is_not_paused : unit = Common.assert_is_not_paused store in
    ...
    | Some x -> x 
    in
    let _check_reserve_auction : unit = Common.assert_auction_reserve_price_not_zero currentAuction in
    // case first bid
    let _check_is_not_expired : unit = Common.assert_is_not_expired currentAuction in
    ...
    let modified_auctions, operations = 
        if (currentAuction.endTime = (None : timestamp option)) then
            ...
            // case not first bid
            let _check_auction_is_over : unit = Common.assert_auction_is_not_over(currentAuction) in
            let _check_auction_no_double_bid : unit = Common.assert_no_twice_bidding(currentAuction) in
            let _check_assert_bid_is_enough : unit = Common.assert_bid_is_enough(currentAuction, store) in
            ...
            let old_bidder = match currentAuction.bidderAddress with
                | None -> failwith("no bidderAddress")
                | Some addr -> addr
            in
            ...
            | None -> (failwith("SHOULD NEVER OCCUR: unknown bidder address") : unit contract)
            | Some dest -> dest
            in
...
let finalizeAuction(param, store : Parameter.finalize_auction_param * Storage.t) : return =
    ...
    | Some x -> x
    in
    let _check_auction_not_started : unit = Common.assert_auction_is_started currentAuction in
    let _check_auction_not_ended : unit = Common.assert_auction_is_not_ended currentAuction in
    ...
    | None -> (failwith("No bidder address") : address)
    | Some addr -> addr
    in
    ...
    | None -> (failwith("SHOULD NEVER OCCUR: unknown reserve address") : unit contract)
    | Some dest -> dest
    in
    ...
    | None -> (failwith("SHOULD NEVER OCCUR: unknown seller address") : unit contract)
    | Some dest -> dest
    in
...
let setNftAuction(param, store : Parameter.set_nft_auction_param * Storage.t) : return =
    let sender = Tezos.get_sender() in
    let _check_is_not_paused : unit = Common.assert_is_not_paused store in
    let _check_nft_address : unit = Common.assert_nft_contract_exist(param.nftAddress, param.assetClass) in
    let _check_bidding_period_not_zero : unit = Common.assert_bidding_period_not_zero param.auctionBiddingPeriod in
    let _check_expiration_period_not_zero : unit = Common.assert_expiration_period_not_zero param.auctionExpirationPeriod in
    let _check_reserve_price_not_zero : unit = Common.assert_reserve_price_not_zero param.reservePrice in
...
let cancelNftAuction(param, store : Parameter.cancel_nft_auction_param * Storage.t) : return =
    ...
    let _check_auction_not_started : unit = Common.assert_auction_is_not_started currentAuction in
...
let adminCancelNftAuction(param, store : Parameter.admin_cancel_nft_auction_param * Storage.t) : return =
    let _check_admin : unit = Common.assert_only_admin store in
    let _check_reason : unit = Common.assert_reason_is_given param.reason in
    let currentAuction : Storage.nftauction = match Big_map.find_opt param.saleId store.auctionIdToAuction with
    | None -> failwith(Errors.unknown_auction_id)
    | Some x -> x
    in
    let _check_reserve_price : unit = Common.assert_auction_reserve_price_not_zero currentAuction in
