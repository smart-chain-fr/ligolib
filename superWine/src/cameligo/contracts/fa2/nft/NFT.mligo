...
module Storage = struct
   ...
   let set_usage (s:t) (usage:TokenUsage.t) = {s with token_usage = usage}
   let get_usage_of (s:t) (token_id : token_id) : nat =
      match (Big_map.find_opt token_id s.token_usage) with
...
let transfer (t:transfer) (s:storage) : operation list * storage = 
   ...
   // update token_usage
   let process_atomic_usage (usage, t:TokenUsage.t * atomic_trans) = TokenUsage.update_usage_token usage t.token_id in
   let update_usage_single_transfer (usage, t:TokenUsage.t * transfer_from ) = List.fold process_atomic_usage t.tx usage in
   let usage = List.fold update_usage_single_transfer t s.token_usage in
   let s = Storage.set_usage s usage in
...
let mint (param: mint_param) (s: storage) : operation list * storage =
   ...
   let _ = assert_with_error (Tezos.sender = s.admin) Errors.only_admin in
   // update token_ids
...
type parameter = [@layout:comb] | Transfer of transfer | Balance_of of balance_of | Update_operators of update_operators | Mint of mint_param
