...
let assert_eventsBetMap (ctr_taddr : (TYPES.action, TYPES.storage) typed_address) (expected : (nat, TYPES.eventBets) map) : unit =
    let ctr_storage = Test.get_storage(ctr_taddr) in
    let ctr_value : (nat, TYPES.eventBets) map = (ctr_storage.events_bets) in
    if (ctr_value = expected)
        then Test.log("OK", ctr_value)
        else failwith("NOT OK", ctr_value)